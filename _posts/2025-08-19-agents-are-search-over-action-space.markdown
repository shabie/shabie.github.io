...
This also clarifies where agents _shouldn't_ be used. If you have very few options and each comes with a significant, irreversible cost, an agent is the wrong tool for the job. In those cases, the guardrails afforded by a pre-defined automation workflow (i.e. not letting agents run wild) are not a limitation; they are a necessary safeguard.

So, the recipe for success is clear: antifragile agents are a search over the action space that create value when the action space is vast, the cost of exploration is low, and the primary challenge is discovery, not execution.